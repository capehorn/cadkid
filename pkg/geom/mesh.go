...
func NewTriangleMeshWithBox(triangles []*Triangle, box *Box) *Mesh {
	return &Mesh{triangles, nil, box}
}
...
func (m *Mesh) SplitTriangles(maxEdgeLength float64) {
	...
	split = func(t *Triangle) {
		...
		if max <= maxEdgeLength {
			...
			v := InterpolateVertices(v1, v2, v3, VectorW{0.5, 0.5, 0, 1})
			t1 := NewTriangle(v3, v1, v)
			t2 := NewTriangle(v2, v3, v)
			split(t1)
			split(t2)
		} else if d23 == max {
			v := InterpolateVertices(v1, v2, v3, VectorW{0, 0.5, 0.5, 1})
			t1 := NewTriangle(v1, v2, v)
			t2 := NewTriangle(v3, v1, v)
			split(t1)
			split(t2)
		} else {
			v := InterpolateVertices(v1, v2, v3, VectorW{0.5, 0, 0.5, 1})
